...
var (
	...
	cacheHomestayIdPrefix = "cache:homestay:id:"
)
...
func (m *defaultHomestayModel) Insert(ctx context.Context, session sqlx.Session, data *Homestay) (sql.Result, error) {
	...
	homestayIdKey := fmt.Sprintf("%s%v", cacheHomestayIdPrefix, data.Id)
...
func (m *defaultHomestayModel) FindOne(ctx context.Context, id int64) (*Homestay, error) {
	homestayIdKey := fmt.Sprintf("%s%v", cacheHomestayIdPrefix, id)
	var resp Homestay
	err := m.QueryRowCtx(ctx, &resp, homestayIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultHomestayModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheHomestayIdPrefix, primary)
}
