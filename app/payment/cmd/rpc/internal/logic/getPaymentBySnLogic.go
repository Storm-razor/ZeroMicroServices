...
func (l *GetPaymentBySnLogic) GetPaymentBySn(in *pb.GetPaymentBySnReq) (*pb.GetPaymentBySnResp, error) {
	thirdPatment, err := l.svcCtx.ThirdPaymentModel.FindOneBySn(l.ctx, in.Sn)
	...
	if thirdPatment != nil {
		_ = copier.Copy(&resp, thirdPatment)

		resp.CreateTime = thirdPatment.CreateTime.Unix()
		resp.UpdateTime = thirdPatment.UpdateTime.Unix()
		resp.PayTime = thirdPatment.PayTime.Unix()
	}
